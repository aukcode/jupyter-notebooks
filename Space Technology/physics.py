...
Edited by: Howard C. Shaw III
To make it Python 3 compatible for use in Jupyter.
"""
...
class NumberDict(dict):
    ...
    def __rsub__(self, other):
        sum_dict = NumberDict()
        for key in list(other.keys()):
            sum_dict[key] = other[key]
        for key in list(self.keys()):
            sum_dict[key] = sum_dict[key] - self[key]
        return sum_dict
